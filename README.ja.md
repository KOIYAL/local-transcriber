...
- 音声・動画ファイルのドラッグ＆ドロップ
- アプリ内でのマイク録音（会議をその場で録音し、停止と同時に文字起こしへ。録音中は数秒ごとに端末内へ保存されるため、途中でクラッシュしても直前までの音声が残ります。音声は外部に送信されません）
- タスクトレイ常駐（待機・録音中・一時停止・文字起こし中を形の違うアイコンで表示。トレイメニューから録音の開始・停止・一時停止ができ、録音中に終了しようとすると警告します）
...
- 書き出し先フォルダの指定（トレイメニューで一度選べば、以後はダイアログなしでそこへ保存）
- 初回セットアップ後のオフライン利用

![アプリ内録音（経過時間とレベルメーター付き）](docs/screenshots/recording-2-recording.png)
