...
2. **vendor\modelshelf.exe を配置**（gitignore 済みフォルダ）
   ...
   ```bat
   :: A) 公開 Release からダウンロード（認証不要）
   mkdir vendor
   curl -fL https://github.com/KOIYAL/modelshelf/releases/latest/download/modelshelf-windows-x64.exe -o vendor\modelshelf.exe
...
- **macOS 版**: ローカル Mac 不要で **GitHub Actions からビルド可能**
  （Actions → "Build macOS app"）。署名・公証も Secrets 設定で自動化される。
  手順と Secrets 一覧は `docs/CLOUD-MAC-BUILD.md`
...
## 追記（2026-07-11）: 配布は GitHub Releases に移行

0.5.0 から配布チャネルを R2 直配信から **GitHub Releases** に切り替えた。

- リリース = `v<version>` タグを push → `gh release create` でノート作成
- **Windows**: WSL チェックアウト（`~/local-transcriber`）でビルドした zip を
  `gh release upload v<version> dist/LocalTranscriber-<version>-win-x64.zip` で添付
  （Windows 側チェックアウトは廃止済み。ビルド手法はビルド venv
  `C:\Users\alanc\AppData\Local\local-transcriber-build` を interop 経由で使う）
- **macOS**: `v*` タグ push で GitHub Actions（build-macos.yml）が
  `LocalTranscriber-macOS-arm64.dmg` を自動ビルド → run の artifact を
  リリースに添付。署名 Secrets（docs/CLOUD-MAC-BUILD.md）が入るまでは
  未署名テストビルドとして注記付きで公開
- **website**: `src/content.ts` のダウンロード URL を Releases のアセット URL に
  向ける。R2 の旧 zip（0.4.0 / 0.5.0）は既存リンク互換のため残置
